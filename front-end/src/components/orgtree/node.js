...
function createListener (handler, data) {
  const execute = (cb, e, ...args) => {
    if (typeof cb === 'function') {
      ...
      if (typeof className === 'string' && className.indexOf('orgtree-node-btn') > -1) return
...
export function renderLabel(h, data, context) {
  ...
  const cls = ['orgtree-node-label-inner']
...
export function renderChildren(h, list, context) {
  if (Array.isArray(list) && list.length) {
    ...
    return h('div', {
      domProps: {
        className: 'orgtree-node-children'
      }
    }, children)
  }
  return ''
